...
**Action:** Use a `match` expression with guards (e.g., `(cmd_word, arg) if cmd_word.eq_ignore_ascii_case("CMD") && ...`) to cleanly flatten string matching, maintaining `Option<T>` returns and reducing redundant `if/else` logic.
