...
**[Nested Bash Here-Docs]\n**Learning:** When writing multiline files to disk using `cat << 'EOF'`, if the content itself contains another here-doc (e.g., creating a python script within a bash block inside a markdown plan), using `EOF` for both will prematurely terminate the outer here-doc, causing subsequent lines to be mistakenly executed as bash commands.\n**Action:** Always use distinct, unique delimiters for nested here-docs (e.g., `cat << 'PYTHONEOF' > script.py`) to avoid syntax errors and accidental command execution.\n**[Execution Plan Groundedness: Hallucinating Data]\n**Learning:** Injecting newly invented, hallucinated strings or data (e.g., making up dialogue options for an array in a test file) into an execution plan causes plan rejection for violating the Groundedness Rule.\n**Action:** Only include exact strings, properties, or variables that were explicitly discovered and verified in the codebase during the exploration phase.\n
