...
impl GameState {
    ...
    /// ⚡ Bolt Optimization: Removes heap allocation from uppercase conversion when checking notable actions.
    pub fn record_command(&mut self, cmd: &str) {
        let cmd_string = cmd.to_string();

        let cmd_bytes = cmd.as_bytes();
        let is_notable = cmd_bytes
            .windows(4)
            ...
            || cmd_bytes.windows(3).any(|w| w.eq_ignore_ascii_case(b"WHO"));
