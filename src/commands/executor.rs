...
impl CommandExecutor {
    ...
    fn handle_nova_commands(&self, cmd: &str) -> Option<CommandResult> {
        ...
        #[cfg(feature = "nova")]
        match (cmd_word, arg) {
            (c, a) if (c.eq_ignore_ascii_case("SPEAK") || c.eq_ignore_ascii_case("SAY")) && !a.is_empty() => Some(self.handle_nova_speak(a)),
            (c, "") if c.eq_ignore_ascii_case("PS") || c.eq_ignore_ascii_case("TOP") || c.eq_ignore_ascii_case("TASKS") => Some(self.handle_nova_ps()),
            (c, "") if c.eq_ignore_ascii_case("MEMDUMP") || c.eq_ignore_ascii_case("EXPORT") => Some(self.handle_nova_memdump()),
            (c, "") if c.eq_ignore_ascii_case("DIAG") || c.eq_ignore_ascii_case("SYS") => Some(self.handle_nova_diag()),

            #[cfg(feature = "nova")]
            (c, "") if c.eq_ignore_ascii_case("LIFE") || c.eq_ignore_ascii_case("AUTOMATON") => Some(self.handle_nova_life()),
            #[cfg(not(feature = "nova"))]
            (c, "") if c.eq_ignore_ascii_case("LIFE") || c.eq_ignore_ascii_case("AUTOMATON") => None,

            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("ANALYZE") && !a.is_empty() => Some(self.handle_nova_analyze(a)),
            #[cfg(not(feature = "nova"))]
            (c, a) if c.eq_ignore_ascii_case("ANALYZE") && !a.is_empty() => None,

            #[cfg(feature = "nova")]
            (c, "") if c.eq_ignore_ascii_case("MAIL") => Some(self.handle_nova_mail()),
            #[cfg(not(feature = "nova"))]
            (c, "") if c.eq_ignore_ascii_case("MAIL") => None,

            (c, a) if (c.eq_ignore_ascii_case("SEARCH") || c.eq_ignore_ascii_case("FIND")) && !a.is_empty() => Some(self.handle_nova_search(a)),
            (c, "") if c.eq_ignore_ascii_case("DEFRAG") => Some(self.handle_nova_defrag()),
            (c, "") if c.eq_ignore_ascii_case("UNDELETE") || c.eq_ignore_ascii_case("RECOVER") => Some(self.handle_nova_undelete()),
            (c, a) if c.eq_ignore_ascii_case("PING") && !a.is_empty() => Some(self.handle_nova_ping(a)),
            (c, a) if (c.eq_ignore_ascii_case("DIAL") || c.eq_ignore_ascii_case("CALL")) && !a.is_empty() => Some(self.handle_nova_dial(a)),
            (c, a) if (c.eq_ignore_ascii_case("DUMP") || c.eq_ignore_ascii_case("HEXDUMP")) && !a.is_empty() => Some(self.handle_nova_dump(a)),
            (c, a) if (c.eq_ignore_ascii_case("TRACE") || c.eq_ignore_ascii_case("TRACEROUTE")) && !a.is_empty() => Some(self.handle_nova_trace(a)),
            (c, a) if c.eq_ignore_ascii_case("STAT") && !a.is_empty() => Some(self.handle_nova_stat(a)),
            (c, "") if c.eq_ignore_ascii_case("ENV") || c.eq_ignore_ascii_case("PRINTENV") => Some(self.handle_nova_env()),
            (c, "") if c.eq_ignore_ascii_case("NETSTAT") => Some(self.handle_nova_netstat()),
            (c, a) if c.eq_ignore_ascii_case("RADIO") || c.eq_ignore_ascii_case("TUNE") => Some(self.handle_nova_radio(a)),
            (c, "") if c.eq_ignore_ascii_case("SENSORS") || c.eq_ignore_ascii_case("SENSE") || c.eq_ignore_ascii_case("TEMP") => Some(self.handle_nova_sensors()),
            (c, "") if c.eq_ignore_ascii_case("HISTORY") || c.eq_ignore_ascii_case("HIST") => Some(self.handle_nova_history()),
            (c, "") if c.eq_ignore_ascii_case("PROFILE") || c.eq_ignore_ascii_case("ANALYZE") => Some(self.handle_nova_profile()),
            (c, "") if c.eq_ignore_ascii_case("SLEEP") => Some(self.handle_nova_sleep()),
            (c, "") if c.eq_ignore_ascii_case("FORTUNE") => Some(self.handle_nova_fortune()),
            (c, "") if c.eq_ignore_ascii_case("WHOAMI") => Some(self.handle_nova_whoami()),
            _ => None,
        }
    }
