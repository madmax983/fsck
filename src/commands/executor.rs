...
impl CommandExecutor {
    ...
    fn handle_nova_commands(&self, cmd: &str) -> Option<CommandResult> {
        ...
        } else if (cmd_word.eq_ignore_ascii_case("ENTROPY")
            || cmd_word.eq_ignore_ascii_case("SCAN"))
            && arg.is_empty()
        {
            Some(self.handle_nova_entropy())
        } else {
            None
        }
    }
    ...
    #[cfg(feature = "nova")]
    fn handle_nova_entropy(&self) -> CommandResult {
        let mut entropy_output =
            crate::experimental::EntropyScanner::scan_entropy(&self.entity, 0xF5C0_0000);
        // ⚡ Bolt Optimization: Append newline directly instead of allocating a new string via format!
        entropy_output.push('\n');
        CommandResult::success(entropy_output)
    }
