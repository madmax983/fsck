...
impl CommandExecutor {
    ...
    fn handle_nova_commands(&self, cmd: &str) -> Option<CommandResult> {
        ...
        match (cmd_word, arg) {
            ...
            (c, _) if (c.eq_ignore_ascii_case("LIFE") || c.eq_ignore_ascii_case("AUTOMATON")) => {
                None
            }
            #[cfg(feature = "nova")]
            ...
            #[cfg(not(feature = "nova"))]
            (c, _) if c.eq_ignore_ascii_case("ANALYZE") => None,
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("MAIL") && a.is_empty() => {
                Some(self.handle_nova_mail())
            }
            #[cfg(not(feature = "nova"))]
            (c, _) if c.eq_ignore_ascii_case("MAIL") => None,
            (c, a)
            ...
            (c, a)
                if (c.eq_ignore_ascii_case("PROFILE") || c.eq_ignore_ascii_case("ANALYZE"))
                    && a.is_empty() =>
            {
