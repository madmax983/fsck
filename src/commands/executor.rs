...
impl CommandExecutor {
    ...
    #[cfg(feature = "nova")]
    fn handle_experimental_command(&self, cmd: &str) -> Option<CommandResult> {
        ...
        if cmd_upper.starts_with("SEARCH ") || cmd_upper.starts_with("FIND ") {
            if let Some((_, query)) = cmd.split_once(' ') {
        ...
        if cmd_upper.starts_with("DUMP ") || cmd_upper.starts_with("HEXDUMP ") {
            if let Some((_, filename)) = cmd_upper.split_once(' ') {
    ...
    fn handle_unknown_command(&self, cmd: &str) -> CommandResult {
        #[cfg(feature = "nova")]
        {
            if let Some(result) = self.handle_experimental_command(cmd) {
                return result;
            }
        }
