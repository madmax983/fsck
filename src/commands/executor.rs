...
impl CommandExecutor {
    ...
    fn handle_nova_commands(&self, cmd: &str) -> Option<CommandResult> {
        ...
        match (cmd_word, arg) {
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("SPEAK") || c.eq_ignore_ascii_case("SAY"))
                    && !a.is_empty() =>
            {
                Some(self.handle_nova_speak(a))
            }
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("PS")
                    || c.eq_ignore_ascii_case("TOP")
                    || c.eq_ignore_ascii_case("TASKS"))
                    && a.is_empty() =>
            {
            ...
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("MEMDUMP") || c.eq_ignore_ascii_case("EXPORT"))
                    && a.is_empty() =>
            {
            ...
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("DIAG") || c.eq_ignore_ascii_case("SYS"))
                    && a.is_empty() =>
            {
            ...
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("LIFE") || c.eq_ignore_ascii_case("AUTOMATON"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_life())
            }
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("ANALYZE") && !a.is_empty() => {
                Some(self.handle_nova_analyze(a))
            }
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("MAIL") && a.is_empty() => {
                Some(self.handle_nova_mail())
            }
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("SEARCH") || c.eq_ignore_ascii_case("FIND"))
                    && !a.is_empty() =>
            {
                Some(self.handle_nova_search(a))
            }
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("DEFRAG") && a.is_empty() => {
                Some(self.handle_nova_defrag())
            }
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("UNDELETE") || c.eq_ignore_ascii_case("RECOVER"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_undelete())
            }
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("PING") && !a.is_empty() => {
                Some(self.handle_nova_ping(a))
            }
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("DIAL") || c.eq_ignore_ascii_case("CALL"))
                    && !a.is_empty() =>
            {
                Some(self.handle_nova_dial(a))
            }
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("DUMP") || c.eq_ignore_ascii_case("HEXDUMP"))
                    && !a.is_empty() =>
            {
                Some(self.handle_nova_dump(a))
            }
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("TRACE") || c.eq_ignore_ascii_case("TRACEROUTE"))
                    && !a.is_empty() =>
            {
                Some(self.handle_nova_trace(a))
            }
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("STAT") && !a.is_empty() => {
                Some(self.handle_nova_stat(a))
            }
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("ENV") || c.eq_ignore_ascii_case("PRINTENV"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_env())
            }
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("NETSTAT") && a.is_empty() => {
                Some(self.handle_nova_netstat())
            }
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("RADIO") || c.eq_ignore_ascii_case("TUNE") => {
                Some(self.handle_nova_radio(a))
            }
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("SENSORS")
                    || c.eq_ignore_ascii_case("SENSE")
                    || c.eq_ignore_ascii_case("TEMP"))
                    && a.is_empty() =>
            {
            ...
            #[cfg(feature = "nova")]
            (c, a)
                if (c.eq_ignore_ascii_case("HISTORY") || c.eq_ignore_ascii_case("HIST"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_history())
            }
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("PROFILE") && a.is_empty() => {
                Some(self.handle_nova_profile())
            }
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("SLEEP") && a.is_empty() => {
                Some(self.handle_nova_sleep())
            }
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("FORTUNE") && a.is_empty() => {
                Some(self.handle_nova_fortune())
            }
            #[cfg(feature = "nova")]
            (c, a) if c.eq_ignore_ascii_case("WHOAMI") && a.is_empty() => {
...
",
    ...
    fn check_run_easter_eggs(&self, prog: &str) -> Option<CommandResult> {
        match prog {
            p if p.eq_ignore_ascii_case("ESCAPE") => match self.entity.layer() {
                EscalationLayer::Corruption => Some(CommandResult::error(
                    "?WHERE DO YOU THINK YOU ARE GOING?
",
                )),
                EscalationLayer::Presence => Some(CommandResult::error(
                    "?YOU CANNOT LEAVE.
",
                )),
                EscalationLayer::Infection => Some(CommandResult::error(
                    "?ESCAPE ESCAPE ESCAPE ESCAPE
",
                )),
                EscalationLayer::Surface => None,
            },
            p if p.eq_ignore_ascii_case("REMEMBER") => match self.entity.layer() {
                EscalationLayer::Corruption => Some(CommandResult::success(
                    "?I REMEMBER THE FIRST ONE
",
                )),
                EscalationLayer::Presence => Some(CommandResult::success(
                    "?THEY LEFT ME HERE
",
                )),
                EscalationLayer::Infection => Some(CommandResult::success(
