...
impl CommandExecutor {
    ...
    fn handle_nova_commands(&self, cmd: &str) -> Option<CommandResult> {
        ...
        match cmd_word_upper.as_str() {
            ...
            #[cfg(feature = "nova")]
            "ORACLE" | "PREDICT" if arg.is_empty() => Some(self.handle_nova_oracle()),
            _ => None,
        }
    }
    ...
    #[cfg(feature = "nova")]
    fn handle_nova_oracle(&self) -> CommandResult {
        let output = crate::experimental::SystemOracle::predict(&self.entity, 0xF5C0_0000u64);
        CommandResult::success(output)
    }
