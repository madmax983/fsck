...
impl CommandExecutor {
    ...
    fn handle_nova_commands(&self, cmd: &str) -> Option<CommandResult> {
        ...
        match (cmd_word, arg) {
            #[cfg(feature = "nova")]
            (cw, a)
                if (cw.eq_ignore_ascii_case("SPEAK") || cw.eq_ignore_ascii_case("SAY"))
                    && !a.is_empty() =>
            {
                Some(self.handle_nova_speak(a))
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("PS")
                    || cw.eq_ignore_ascii_case("TOP")
                    || cw.eq_ignore_ascii_case("TASKS"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_ps())
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("MEMDUMP") || cw.eq_ignore_ascii_case("EXPORT"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_memdump())
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("DIAG") || cw.eq_ignore_ascii_case("SYS"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_diag())
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("LIFE") || cw.eq_ignore_ascii_case("AUTOMATON"))
                    && a.is_empty() =>
            {
                #[cfg(feature = "nova")]
                {
            ...
            (cw, a) if cw.eq_ignore_ascii_case("ANALYZE") && !a.is_empty() => {
                #[cfg(feature = "nova")]
                {
            ...
            (cw, a) if cw.eq_ignore_ascii_case("MAIL") && a.is_empty() => {
                #[cfg(feature = "nova")]
                {
            ...
            (cw, a)
                if (cw.eq_ignore_ascii_case("SEARCH") || cw.eq_ignore_ascii_case("FIND"))
                    && !a.is_empty() =>
            {
                Some(self.handle_nova_search(a))
            }
            (cw, a) if cw.eq_ignore_ascii_case("DEFRAG") && a.is_empty() => {
                Some(self.handle_nova_defrag())
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("UNDELETE") || cw.eq_ignore_ascii_case("RECOVER"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_undelete())
            }
            (cw, a) if cw.eq_ignore_ascii_case("PING") && !a.is_empty() => {
                Some(self.handle_nova_ping(a))
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("DIAL") || cw.eq_ignore_ascii_case("CALL"))
                    && !a.is_empty() =>
            {
                Some(self.handle_nova_dial(a))
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("DUMP") || cw.eq_ignore_ascii_case("HEXDUMP"))
                    && !a.is_empty() =>
            {
                Some(self.handle_nova_dump(a))
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("TRACE") || cw.eq_ignore_ascii_case("TRACEROUTE"))
                    && !a.is_empty() =>
            {
                Some(self.handle_nova_trace(a))
            }
            (cw, a) if cw.eq_ignore_ascii_case("STAT") && !a.is_empty() => {
                Some(self.handle_nova_stat(a))
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("ENV") || cw.eq_ignore_ascii_case("PRINTENV"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_env())
            }
            (cw, a) if cw.eq_ignore_ascii_case("NETSTAT") && a.is_empty() => {
                Some(self.handle_nova_netstat())
            }
            (cw, a) if cw.eq_ignore_ascii_case("RADIO") || cw.eq_ignore_ascii_case("TUNE") => {
                Some(self.handle_nova_radio(a))
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("SENSORS")
                    || cw.eq_ignore_ascii_case("SENSE")
                    || cw.eq_ignore_ascii_case("TEMP"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_sensors())
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("HISTORY") || cw.eq_ignore_ascii_case("HIST"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_history())
            }
            (cw, a)
                if (cw.eq_ignore_ascii_case("PROFILE") || cw.eq_ignore_ascii_case("ANALYZE"))
                    && a.is_empty() =>
            {
                Some(self.handle_nova_profile())
            }
            (cw, a) if cw.eq_ignore_ascii_case("SLEEP") && a.is_empty() => {
                Some(self.handle_nova_sleep())
            }
            (cw, a) if cw.eq_ignore_ascii_case("FORTUNE") && a.is_empty() => {
                Some(self.handle_nova_fortune())
            }
            (cw, a) if cw.eq_ignore_ascii_case("WHOAMI") && a.is_empty() => {
