...
impl CommandExecutor {
    ...
    fn handle_nova_commands(&self, cmd: &str) -> Option<CommandResult> {
        ...
        #[cfg(feature = "nova")]
        match (cmd_word, arg) {
            (c, a) if (c.eq_ignore_ascii_case("SPEAK") || c.eq_ignore_ascii_case("SAY")) && !a.is_empty() => {
                Some(self.handle_nova_speak(a))
            }
            (c, a) if (c.eq_ignore_ascii_case("PS") || c.eq_ignore_ascii_case("TOP") || c.eq_ignore_ascii_case("TASKS")) && a.is_empty() => {
                Some(self.handle_nova_ps())
            }
            (c, a) if (c.eq_ignore_ascii_case("MEMDUMP") || c.eq_ignore_ascii_case("EXPORT")) && a.is_empty() => {
                Some(self.handle_nova_memdump())
            }
            (c, a) if (c.eq_ignore_ascii_case("DIAG") || c.eq_ignore_ascii_case("SYS")) && a.is_empty() => {
                Some(self.handle_nova_diag())
            }
            (c, a) if (c.eq_ignore_ascii_case("LIFE") || c.eq_ignore_ascii_case("AUTOMATON")) && a.is_empty() => {
                #[cfg(feature = "nova")]
                {
            ...
            (c, a) if (c.eq_ignore_ascii_case("SEARCH") || c.eq_ignore_ascii_case("FIND")) && !a.is_empty() => {
                Some(self.handle_nova_search(a))
            }
            (c, a) if c.eq_ignore_ascii_case("DEFRAG") && a.is_empty() => {
                Some(self.handle_nova_defrag())
            }
            (c, a) if (c.eq_ignore_ascii_case("UNDELETE") || c.eq_ignore_ascii_case("RECOVER")) && a.is_empty() => {
                Some(self.handle_nova_undelete())
            }
            (c, a) if c.eq_ignore_ascii_case("PING") && !a.is_empty() => {
                Some(self.handle_nova_ping(a))
            }
            (c, a) if (c.eq_ignore_ascii_case("DIAL") || c.eq_ignore_ascii_case("CALL")) && !a.is_empty() => {
                Some(self.handle_nova_dial(a))
            }
            (c, a) if (c.eq_ignore_ascii_case("DUMP") || c.eq_ignore_ascii_case("HEXDUMP")) && !a.is_empty() => {
                Some(self.handle_nova_dump(a))
            }
            (c, a) if (c.eq_ignore_ascii_case("TRACE") || c.eq_ignore_ascii_case("TRACEROUTE")) && !a.is_empty() => {
                Some(self.handle_nova_trace(a))
            }
            (c, a) if c.eq_ignore_ascii_case("STAT") && !a.is_empty() => {
                Some(self.handle_nova_stat(a))
            }
            (c, a) if (c.eq_ignore_ascii_case("ENV") || c.eq_ignore_ascii_case("PRINTENV")) && a.is_empty() => {
            ...
            (c, a) if (c.eq_ignore_ascii_case("SENSORS") || c.eq_ignore_ascii_case("SENSE") || c.eq_ignore_ascii_case("TEMP")) && a.is_empty() => {
                Some(self.handle_nova_sensors())
            }
            (c, a) if (c.eq_ignore_ascii_case("HISTORY") || c.eq_ignore_ascii_case("HIST")) && a.is_empty() => {
                Some(self.handle_nova_history())
            }
            (c, a) if (c.eq_ignore_ascii_case("PROFILE") || c.eq_ignore_ascii_case("ANALYZE")) && a.is_empty() => {
