...
impl CommandExecutor {
    ...
    fn handle_nova_commands(&self, cmd: &str) -> Option<CommandResult> {
        ...
        match cmd_word {
            c if (c.eq_ignore_ascii_case("SPEAK") || c.eq_ignore_ascii_case("SAY")) && !arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_speak(arg)) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("PS") || c.eq_ignore_ascii_case("TOP") || c.eq_ignore_ascii_case("TASKS")) && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_ps()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("MEMDUMP") || c.eq_ignore_ascii_case("EXPORT")) && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_memdump()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("DIAG") || c.eq_ignore_ascii_case("SYS")) && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_diag()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("LIFE") || c.eq_ignore_ascii_case("AUTOMATON")) && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_life()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if c.eq_ignore_ascii_case("ANALYZE") && !arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_analyze(arg)) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if c.eq_ignore_ascii_case("MAIL") && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_mail()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("SEARCH") || c.eq_ignore_ascii_case("FIND")) && !arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_search(arg)) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if c.eq_ignore_ascii_case("DEFRAG") && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_defrag()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("UNDELETE") || c.eq_ignore_ascii_case("RECOVER")) && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_undelete()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if c.eq_ignore_ascii_case("PING") && !arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_ping(arg)) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("DIAL") || c.eq_ignore_ascii_case("CALL")) && !arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_dial(arg)) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("DUMP") || c.eq_ignore_ascii_case("HEXDUMP")) && !arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_dump(arg)) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("TRACE") || c.eq_ignore_ascii_case("TRACEROUTE")) && !arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_trace(arg)) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if c.eq_ignore_ascii_case("STAT") && !arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_stat(arg)) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("ENV") || c.eq_ignore_ascii_case("PRINTENV")) && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_env()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if c.eq_ignore_ascii_case("NETSTAT") && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_netstat()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if c.eq_ignore_ascii_case("RADIO") || c.eq_ignore_ascii_case("TUNE") => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_radio(arg)) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("SENSORS") || c.eq_ignore_ascii_case("SENSE") || c.eq_ignore_ascii_case("TEMP")) && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_sensors()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("HISTORY") || c.eq_ignore_ascii_case("HIST")) && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_history()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if (c.eq_ignore_ascii_case("PROFILE") || c.eq_ignore_ascii_case("ANALYZE")) && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_profile()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if c.eq_ignore_ascii_case("SLEEP") && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_sleep()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if c.eq_ignore_ascii_case("FORTUNE") && arg.is_empty() => {
                #[cfg(feature = "nova")]
                { Some(self.handle_nova_fortune()) }
                #[cfg(not(feature = "nova"))]
                { None }
            }
            c if c.eq_ignore_ascii_case("WHOAMI") && arg.is_empty() => {
                #[cfg(feature = "nova")]
