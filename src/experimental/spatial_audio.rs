...
impl SpatialAudioGenerator {
    ...
    pub fn generate_anomaly(entity: &Entity, base_seed: u64) -> Option<String> {
        ...
        let hint = match layer {
            EscalationLayer::Surface => Self::get_surface_hint(&mut rng),
            EscalationLayer::Corruption => Self::get_corruption_hint(&mut rng),
            EscalationLayer::Presence => Self::get_presence_hint(&mut rng),
            EscalationLayer::Infection => Self::get_infection_hint(&mut rng),
        };

        Some(format!("*[AUDIO ANOMALY: {hint}]*"))
    }

    fn get_surface_hint(rng: &mut ChaCha8Rng) -> &'static str {
        let hints = [
    ...
    fn get_corruption_hint(rng: &mut ChaCha8Rng) -> &'static str {
        let hints = [
    ...
    fn get_presence_hint(rng: &mut ChaCha8Rng) -> &'static str {
        let hints = [
    ...
    fn get_infection_hint(rng: &mut ChaCha8Rng) -> &'static str {
        let hints = [
