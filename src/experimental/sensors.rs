...
impl HardwareSensors {
    ...
    pub fn get_readings(entity: &Entity, base_seed: u64) -> String {
        ...
        match layer {
            EscalationLayer::Surface => Self::generate_surface_readings(&mut output, &mut rng),
            EscalationLayer::Corruption => {
                Self::generate_corruption_readings(&mut output, &mut rng);
            }
            EscalationLayer::Presence => Self::generate_presence_readings(&mut output, &mut rng),
            EscalationLayer::Infection => Self::generate_infection_readings(&mut output, &mut rng),
        }

        output
    }

    fn generate_surface_readings(output: &mut String, rng: &mut ChaCha8Rng) {
    ...
    fn generate_corruption_readings(output: &mut String, rng: &mut ChaCha8Rng) {
    ...
    fn generate_presence_readings(output: &mut String, rng: &mut ChaCha8Rng) {
    ...
    fn generate_infection_readings(output: &mut String, rng: &mut ChaCha8Rng) {
