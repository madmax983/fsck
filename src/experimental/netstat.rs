...
impl NetStatGenerator {
    ...
    fn generate_connection(output: &mut String, rng: &mut ChaCha8Rng, layer: EscalationLayer) {
        ...
        let (state, remote_addr) = match layer {
            EscalationLayer::Surface => {
                Self::generate_surface_connection(rng, remote_port, &normal_states)
            }
            EscalationLayer::Corruption => Self::generate_corruption_connection(
                rng,
                remote_port,
                &normal_states,
                &corrupted_states,
            ),
            EscalationLayer::Presence => Self::generate_presence_connection(
                rng,
                remote_port,
                &normal_states,
                &corrupted_states,
            ...
            EscalationLayer::Infection => {
                Self::generate_infection_connection(rng, &presence_states, &infection_states)
            }
        };
    ...
    fn generate_surface_connection(
        rng: &mut ChaCha8Rng,
        remote_port: u16,
        normal_states: &[&'static str],
    ) -> (&'static str, String) {
    ...
    fn generate_corruption_connection(
        rng: &mut ChaCha8Rng,
        remote_port: u16,
        normal_states: &[&'static str],
        corrupted_states: &[&'static str],
    ) -> (&'static str, String) {
        if rng.gen_bool(0.4) {
    ...
    fn generate_presence_connection(
        rng: &mut ChaCha8Rng,
        remote_port: u16,
        normal_states: &[&'static str],
        corrupted_states: &[&'static str],
        presence_states: &[&'static str],
    ) -> (&'static str, String) {
    ...
    fn generate_infection_connection(
        rng: &mut ChaCha8Rng,
        presence_states: &[&'static str],
        infection_states: &[&'static str],
    ) -> (&'static str, String) {
        if rng.gen_bool(0.7) {
